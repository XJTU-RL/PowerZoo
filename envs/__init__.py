from absl import flags
from envs.smac.smac_logger import SMACLogger
from envs.smacv2.smacv2_logger import SMACv2Logger
from envs.mamujoco.mamujoco_logger import MAMuJoCoLogger
from envs.pettingzoo_mpe.pettingzoo_mpe_logger import PettingZooMPELogger
from envs.gym.gym_logger import GYMLogger
from envs.football.football_logger import FootballLogger
from envs.dexhands.dexhands_logger import DexHandsLogger
from envs.lag.lag_logger import LAGLogger
from envs.powerzoo.powerzoo_logger import PowerZooLogger

FLAGS = flags.FLAGS
FLAGS(["train_sc.py"])

LOGGER_REGISTRY = {
    "smac": SMACLogger,
    "mamujoco": MAMuJoCoLogger,
    "pettingzoo_mpe": PettingZooMPELogger,
    "gym": GYMLogger,
    "football": FootballLogger,
    "dexhands": DexHandsLogger,
    "smacv2": SMACv2Logger,
    "lag": LAGLogger,
    "powerzoo":PowerZooLogger,
}
