...
import re
try:
    from envs.power_envs.powerzoo_llm.base_env.env import Env
except ImportError:
    # 相对导入用于测试
    from .env import Env
...
def get_info_from_config(env_name, config_dict=None):
    ...
    if config_dict and 'environment_specific' in config_dict:
        ...
        # 优先从顶层配置获取dss_file（支持完整路径），然后是environment_specific
        dss_file = config_dict.get('dss_file') or env_config.get('dss_file', 'ieee34Mod1_duty.dss')
        
        # 如果dss_file包含完整路径，从中提取system_name和dss_file
        if dss_file.startswith('./node_systems/') and '/' in dss_file:
            # 解析路径: "./node_systems/34Bus_PV_Aggressive/ieee34Mod1_duty.dss"
            path_parts = dss_file.split('/')
            if len(path_parts) >= 4:  # ['.' 'node_systems' '34Bus_PV_Aggressive' 'ieee34Mod1_duty.dss']
                extracted_system_name = path_parts[2]  # '34Bus_PV_Aggressive'
