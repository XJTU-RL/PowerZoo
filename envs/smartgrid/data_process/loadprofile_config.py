...
from envs.smartgrid.data_process.loadprofile_dss_parser import Constants
        ...
        try:
            with open(pv_dss_path, 'w', encoding='utf-8') as fp:
                ...
                if has_pv_data:
                    ...
                    fp.write(f'New Loadshape.MyIrrad npts={self.steps} sinterval=60 mult=(file=./{relative_irr_path}) useactual=yes\n')
                else:
                    # 如果没有PV数据，使用默认值
                    fp.write('! 使用默认辐照度LoadShape (无数据文件)\n')
                    fp.write(f'New Loadshape.MyIrrad npts={self.steps} sinterval=60 mult=1.0 useactual=yes\n')
                fp.write('\n')
                ...
                if has_temp_data:
                    ...
                    fp.write(f'New TShape.MyTemp npts={self.steps} sinterval=60 temp=(file=./{relative_temp_path}) useactual=yes\n')
                else:
                    # 如果没有温度数据，使用默认值
