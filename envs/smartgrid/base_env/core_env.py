...
@File      : core_env.py
@Time      : 2025-12-24
@Author    : Xiaodong Zheng (adapted for smartgrid)
@Email     : zxd_xjtu@stu.xjtu.edu.cn
@Description: Core environment classes for SmartGrid - adapted from legacy PowerZoo
              Contains ActionSpace and Env base classes with SmartGrid module imports.

- Helper Functions:
  - `plotting`: Plot network status with load profile and save as image
  - `FFT_selection`: Farthest-first traversal algorithm for battery location selection
  - `choose_batteries`: Choose battery installation locations based on environment state
  - `get_basekv`: Get base voltage for specified bus

- `ActionSpace` class: Encapsulates action spaces for capacitors, regulators, and batteries

- `Env` class: Base environment class for training RL agents
  - Initialize environment parameters (DSS file paths, load profiles, circuit topology)
  - Reset observation space based on wrapping and load options
  - Define reward function (power loss, control actions, voltage violations)
  - Implement `step` and `reset` methods
  - Provide plotting functionality for visualization
