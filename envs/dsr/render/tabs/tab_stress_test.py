...
支持参数扫描 (n_faults, fault_severity, load_priority_ratio,
max_episode_steps, load_noise) 和灵敏度分析，
展示 DSR 在不同故障条件下的恢复表现。
"""
...
def create_tab(shared_states: Dict[str, Any]) -> Dict[str, Any]:
	...
	with gr.Tab("Stress Test"):
		...
		gr.Markdown("### DSR Stress Test & Sensitivity Analysis")
		gr.Markdown(
			"Sweep parameters: `n_faults` (1~10), "
			"`fault_severity` (0.1~1.0), `load_priority_ratio` (0.0~1.0), "
			"`max_episode_steps` (5~50)"
		)

		with gr.Row():
			param_dropdown = gr.Dropdown(
				label="Sweep Parameter",
				choices=["n_faults", "fault_severity", "load_priority_ratio",
						 "max_episode_steps", "load_noise"],
				value="n_faults",
			)
			sweep_min = gr.Number(label="Min", value=1)
			sweep_max = gr.Number(label="Max", value=10)
		...
		with gr.Row():
			...
			max_steps_input = gr.Number(label="Max Steps", value=20, precision=0)
	...
	def _on_param_change(param_name: str):
		"""参数选择变化时更新默认范围"""
		defaults = DSRStressTestRunner.SWEEP_DEFAULTS
...
def _build_sweep_chart(
	...
	DSR 特色: 双 Y 轴展示 total_reward 和 restoration_pct。
	"""
	...
	rest_pcts = [r["metrics"].get("final_restoration_pct", 0) for r in results]
	...
	if any(v > 0 for v in rest_pcts):
		fig.add_trace(go.Scatter(
			x=param_vals, y=rest_pcts,
