...
存储格式: .npz 压缩文件
- metadata: JSON string (algorithm, seed, config, timestamp)
- n_steps: int
- step_{i}_buses: JSON string (序列化 dict)
- step_{i}_lines: JSON string (序列化 dict)
- step_{i}_devices: JSON string (序列化 dict)
- step_{i}_circuit: JSON string (序列化 dict)
- step_{i}_actions: np.ndarray
- step_{i}_rewards: np.ndarray
"""
...
from envs.district_dispatch.render.engine.episode_reader import EpisodeData
...
class EpisodeRecorder:
	...
	存储格式: .npz 压缩文件，每个时间步的 buses/lines/devices/circuit
	以 JSON 字符串形式存储，actions/rewards 以 numpy 数组形式存储。
	...
	def save_episode(
		...
		Args:
			...
			filename: 自定义文件名（不含扩展名）。默认自动生成
				格式: episode_{timestamp}_{algorithm}_{seed}
	...
	def _serialize_snapshot(snapshot: Dict[str, Any]) -> Dict[str, Any]:
		...
		Dict 类型数据（buses, lines, devices, circuit）转为 JSON 字符串。
		numpy 数组类型数据（actions, rewards）保持原格式。

		Args:
			snapshot: 原始快照字典，可能包含以下键:
				buses, lines, devices, circuit, actions, rewards
