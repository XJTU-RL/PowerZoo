# -*- coding: utf-8 -*-
"""
PowerZoo Stackelberg Render System - Gradio 主入口

基于 Gradio 6.x Blocks 构建的 8-Tab 交互式渲染系统。
针对 Stackelberg 博弈环境的异质智能体 (UC Leader + Consumer Followers)
提供模型加载、实时推理、回放、分析、对比、压测、训练监控和导出功能。
"""
...
def _import_tab(module_path: str, label: str):
	...
	try:
		...
		logger.warning(f"Tab '{label}' import failed: {exc}")
		return None
...
def create_app() -> gr.Blocks:
	...
	重点处理 Stackelberg 博弈的 UC Leader / Consumer Follower 异质智能体。
	...
	tab_modules = {
		"Model & Data": _import_tab(
			"envs.stackelberg.render.tabs.tab_model_data", "Model & Data"
		),
		"Live Inference": _import_tab(
			"envs.stackelberg.render.tabs.tab_live_inference", "Live Inference"
		),
		"Playback": _import_tab(
			"envs.stackelberg.render.tabs.tab_playback", "Playback"
		),
		"Analytics": _import_tab(
			"envs.stackelberg.render.tabs.tab_analytics", "Analytics"
		),
		"Comparison": _import_tab(
			"envs.stackelberg.render.tabs.tab_comparison", "Comparison"
		),
		"Stress Test": _import_tab(
			"envs.stackelberg.render.tabs.tab_stress_test", "Stress Test"
		),
		"Training": _import_tab(
			"envs.stackelberg.render.tabs.tab_training", "Training"
		),
		"Export": _import_tab(
			"envs.stackelberg.render.tabs.tab_export", "Export"
		),
	}

	with gr.Blocks(
		theme=gr.themes.Soft(primary_hue="amber"),
		title="PowerZoo Stackelberg Render",
	) as demo:
		# --- 标题 ---
		gr.Markdown("# PowerZoo Stackelberg - Render System")
		gr.Markdown(
			"Stackelberg game visualization: "
			"**UC Leader** (price/DR/ESS) vs **Consumer Followers** (load/DER/flex)"
		)
